...
static void pls_output_file(FILE *fp, pPLA PLA);
static void eqn_output_file(FILE *fp, pPLA PLA);
...
void fprint_pla(FILE *fp, pPLA PLA, int output_type)
{
	...
	pls_output_file(fp, PLA);
    } else if (output_type == EQNTOTT_type) {
	eqn_output_file(fp, PLA);
...
static void pls_output_file(FILE *fp, pPLA PLA)
{
    ...
    fprintf(fp, ".option unmerged\n");
    makeup_labels(PLA);
    pls_label(PLA, fp);
    pls_group(PLA, fp);
    fprintf(fp, ".p %d\n", PLA->F->count);
    foreach_set(PLA->F, last, p) {
	print_expanded_cube(fp, p, PLA->phase);
    }
    fprintf(fp, ".end\n");
}
...
static void eqn_output_file(FILE *fp, pPLA PLA)
{
	...
	foreach_set(PLA->F, last, p)
		...
		for(var = 0; var < cube.num_binary_vars; var++)
			...
			if (x == ZERO)
			    fprintf(fp, "!"), col += 1;
			fprintf(fp, "%s", INLABEL(var)), col += len;
		    }
		fprintf(fp, ")"), col += 1;
	    }
	fprintf(fp, ";\n\n");
